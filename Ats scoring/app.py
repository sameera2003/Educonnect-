...
def find_not_found_keywords(resume_keywords, job_keywords):
    ...
    return [kw[0] for kw in resume_keywords if kw[0] not in job_keywords_set]

def show_ATS_score(resume_text, job_description):
    resume_keywords = extract_entities(resume_text)
    ...
    return similarity_score, not_found_keywords

def show_entities(text):
    df = pd.DataFrame(extract_entities(text), columns=["Label", "Entity"])
    st.table(df)

def show_suggestion(keywords):
    suggestions = {
        "NAME": "• **Name:** Ensure your full name is clearly stated.",
        "EMAIL_ADDRESS": "• **Email:** Include a professional email for contact.",
        "LOCATION": "• **Location:** Mention your current city or region.",
        "DEGREE": "• **Degree:** List your degrees and certifications.",
        "SKILLS": "• **Skills:** Highlight relevant skills for the job.",
    }
    st.write("#### Suggestions to Improve Your Resume:")
    for kw in keywords:
        if kw in suggestions:
            st.write(suggestions[kw])
...
def main(uploaded_resume, job_description):
    if uploaded_resume is not None:
        ...
        if doc_type == 'application/pdf':
            ...
            text = extract_text_from_txt(uploaded_resume)
        else:
        ...
        if text and job_description:
            ...
            if not_found_keywords:
                st.write('## Suggestions Based on Missing Keywords')
...
#     resume_keywords = extract_entities(resume_text)
#     job_keywords = extract_entities(job_description)
#     cosine_sim = calculate_cosine_similarity(resume_text, job_description)
#     not_found_keywords = find_not_found_keywords(resume_keywords, job_keywords)
#     return cosine_sim, not_found_keywords
...
#     if jobk_count!=0:
#         score=(resumek_count/jobk_count)*10
#         score=round(score,2)
#     else:
#         score=-1
#     not_found_keywords=find_not_found_keywords(resume_keywords,job_keywords)
